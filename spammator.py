...
def main():
    ...
    try:
        ...
        sys.stderr.write(f"error: cannot open {args[0]}")
        sys.exit(1)

    subject = ""
    body = []
    start_msg = False
    for line in mail_f.readlines():
        line = line.rstrip()
    ...
    try:
        ...
        sys.stderr.write(f"error: cannot open {args[1]}\n")
        sys.exit(1)
    ...
    for row in reader:
        ...
        if len(row) != len(header):
            sys.stderr.write(f"ignoring line {counter}: {' '.join(row)}\n")
            continue
    ...
    for email in email_data:
        gmail.send(sender, "{name} <{email}>,".format(**email), subject % email, template % email)
        print("waiting 3 seconds after send an e-mail to {email}...".format(**email))
        time.sleep(3)
